...
int main(){
    ...
    if (mysql_real_connect(conn, "localhost", "group50_dbms", "password", "dbms_lab10", 3306, NULL, 0)) 
    {
        ...
        int queryStatus = writeQuery("DELETE FROM Author_Details WHERE author_id='Ro_015';");
        if (queryStatus==0){
        ...
        {
            cout << "Error in deletion query for part c ! " << mysql_error(conn) << endl;
        }
        
        queryStatus = insertValue(tableName2, auth_det_Columns, {"So_1010", "Soumik"});
        if (queryStatus==0){
            int q_stat = writeQuery("SELECT * FROM income;");
            if(q_stat==0){
                display();
            ...
            cout << "Error in insertion query for part c !" << mysql_error(conn) << endl;
        }
    } 
    else 
    {
